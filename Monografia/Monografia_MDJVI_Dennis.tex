...
\begin{folhadeaprovacao}
  \assinatura{Prof. Dr. Jorge Duarte Pires Valério}
             {Faculdade de Engenharia - UERJ}
  \assinatura{$Prof^a.$ $Dr^a.$ Ana Maria Tavares Cavalcanti}
...
O \textbf{Museu Dom João VI da EBA/UFRJ} foi criado em 1979 com a finalidade de se preservar e catalogar o material que se encontrava distribuído nas salas e ateliês da Escola de Belas Artes e servir como fonte de consulta e estudo sobre a história da arte brasileira. 

A reunião das obras que compõem o Museu deve-se à criação da Escola Real de Ciências, Artes e Ofícios, sancionada por d. João VI em 1816, e ao trabalho das escolas que deram sequência a essa instituição. Joaquim Lebreton (1760-1819), chefe da Missão Artística Francesa, inicia em 1816 o projeto para a Escola Real trazendo consigo da França 42 pinturas atribuídas à Leonardo da Vinci,  Veronese e Poussin, entre outros, especialmente para o fim de constituírem modelos de estudo para a instituição.~\citetext{ebaitau}
...
No segundo capítulo encontram-se um histórico mais detalhado do projeto original realizado no \textbf{Museu D. João VI} e o método escolhido para desenvolver a nova estrutura do banco de dados do mesmo. São relatados os possíveis programas que foram pesquisados e testados e a motivação para escolha final.
...
No quarto capítulo foi detalhada a instalação e uso do Pawtucket, ferramenta utilizada para exibir as informações do banco de dados para o público. Também são exibidas as páginas criadas para o site do Museu e os modos de pesquisa e navegação do acervo.
...
Neste capítulo será apresentado um histórico mais detalhado do banco de dados contendo o acervo do Museu que foi utilizado para o desenvolvimento do projeto. Também será visto o conceito do programa utilizado para gerenciar acervos e coleções, sejam elas de museus ou não, algumas opções disponíveis e o motivo da escolha do Collective Access.
...
Cada um destes bancos de dados possuía dois arquivos diferentes, um com as tabelas com os dados propriamente ditos, e outro com os formulários para entrada e alteração de dados, relatórios e sistema de pesquisa. Esse segundo arquivo ficava no computador ``Mãe'', onde todas as alterações nos dados do acervo eram realizadas.

O foco inicial deste projeto é o gerenciamento e exposição do Acervo Museológico, devido à recente retirada do ar do site que fazia a exibição do mesmo. Porém durante a execução do mesmo foi decidido que o Acervo Arquivístico também seria importado, evitando assim a perda das informações existentes sobre o mesmo. 
...
Também foi dada a preferência por programas de utilização gratuita e que sejam compatíveis com o sistema operacional \textbf{LINUX} para evitar qualquer custo e facilitar a implementação para o Museu Dom João VI. Também foi avaliada a possibilidade de configuração e adaptação para o Acervo, suas fichas já existentes e o suporte oferecido pela empresa desenvolvedora.

\section{Gerenciamento e Publicação de Coleções e Acervos}

Para definir o software a ser utilizado no projeto, os SGC com foco no gerenciamento de coleções e acervos foram estudados, avaliados e testados. Um resumo de cada um deles e os motivos que levaram a sua escolha ou não serão apresentados a seguir.
...
Além de ser necessário adquirir a licença de uso do programa que só funciona em Windows, algumas funcionalidades são vendidas a parte, como a possibilidade de se anexar imagens nos artigos da coleção, conectar outros computadores à base de dados para que se possa realizar as edições na coleção de mais de um local e até mesmo a hospedagem da coleção online.~\citetext{PastPerfect}

\subsection{Argus}

Além da aquisição do programa é necessária a hospedagem do mesmo em conjunto com o banco de dados do acervo em um servidor com Windows que irá controlar todas as operações realizadas no acervo do museu. Essa hospedagem pode ser realizada pelo cliente ou contratada opcionalmente da empresa. 
...
Com foco na exposição online de coleções, o Omeka possui uma das menores curvas de aprendizado, com uma interface simples e fácil instalação e configuração. Permite a utilização de plugins e temas pré configurados, além da instalação e criação de novos conforme necessidade do usuário.~\citetext{OmekaOrg}
...
Sendo uma aplicação baseada em web onde os a equipe do Museu irá acessar o servidor utilizando um login e uma senha, não haverá mais a preocupação de se perder o acesso ao sistema caso ocorra problema em um computador, bastando se ter uma cópia de segurança dos dados do servidor.
...
Para um melhor uso pela equipe do Museu, que pode não ter fluência em inglês, foi utilizado o programa \textit{Poedit} para edição do arquivo com a tradução em português brasileiro. Todas as traduções já existentes foram verificadas e revisadas, e os textos que não continham tradução foram traduzidos. No total, para o Providence, foram traduzidos 6444 textos, que podem ser desde uma palavra até parágrafos inteiros. 

Já para o Pawtucket, que será apresentado no próximo capítulo, foram traduzidos mais 4655 textos. O fato de parte deles ser similar ou idêntica a textos já traduzidos para o Providence auxiliou esta segunda etapa de tradução.
...
\begin{itemize}
	...
	\item \textbf{Inscrições} - Lista os últimos cadastros de usuários feitos pelo público;
...
Na parte superior do site se encontram os menus com as opções para tratamento dos dados e uma barra de pesquisa rápida onde pode-se buscar qualquer tipo de item no banco de dados.
...
\begin{itemize}
	\item \textbf{Objeto} - Utilizado para se criar um novo objeto. Deve-se selecionar a categoria entre Acervo Arquivístico e Acervo Museológico, sendo este segundo dividido nas classes e subclasses configuradas no perfil de instalação;
...
Para o Acervo Arquivístico foi utilizada a tabela Arquivo onde também foi necessário realizar o tratamento dos dados. Para a coluna Notação, que será importada como Número de Registro, foi adicionado o prefixo ``A-'' para diferenciar os registros do Acervo Museológico, evitando assim que o número ficasse duplicado. Caso necessário, o valor original da Notação pode ser adicionada como ``Numeração Original'' na ficha do objeto durante a importação dos valores. As colunas de Data Limite, que estavam separadas em campos separados para dia, mês e ano, foram unidas em uma única coluna. Também foi necessário revisar a incerteza das datas de forma similar a feita anteriormente.

\section{Mapeamento dos dados a serem importados}

Para realizar a importação dos dados para o Providence é necessário primeiro criar uma planilha de mapeamento, que contém as regras e definições como o tipo de informação que será transferida. Foi criada uma planilha para cada categoria da informação que se deseja importar, como por exemplo uma para as Pinturas, outra para as Gravuras, até abranger todas os objetos do Acervo, sendo possível também realizar importações de mídias, pessoas, instituições e qualquer outra informação relevante.

O \textbf{Acervo Museológico} foi separado em 34 arquivos diferentes, um para cada tipo de classe/subclasse existente, logo também foi necessário criar um arquivo de mapeamento para cada um desses arquivos. Já para o \textbf{Acervo Arquivístico} foram criadas três planilhas de mapeamento. A primeira foi utilizada com a tabela Arquivo, mas também foram importadas as tabelas Índice\_Onomástico e Índice\_Temático, que continham informações relevantes e estavam armazenadas de forma separada.

Neste projeto, devido à facilidade de criação e uso, foi decidido utilizar o formato do Excel ``\textit{.xlsx}'' para a elaboração desses mapeamentos.
...
No banco de dados do Projeto Original não foi encontrado nenhum relacionamento existente entre as tabelas. Ao analisar os dados contidos nas mesmas e as informações do Anexo 1, foi possível criar a tabela de relacionamento da Figura \ref{fig:relac_antigo}. Com ela pode-se verificar onde cada informação pertence e deve ser importada. Não foram exibidos os relacionamentos das tabelas auxiliares que geravam os relatórios do sistema pois as mesmas não foram utilizadas.
...
É possível controlar também a interação dos visitantes do site com os objetos, exigindo o cadastro de uma conta para acesso, comentários e curtidas de itens, além de permitir que cada usuário tenha uma coleção própria.
...
Após a configuração correta dos arquivos basta acessar o endereço do servidor no qual foi feita a instalação e o site estará pronto para uso, não sendo necessária nenhuma etapa adicional.
...
Na página do Github do projeto Collective Access, na seção do Pawtucket2 da derivação \textit{develop}, podem ser encontrados diversos temas criados para serem utilizados por instituições que utilizam este programa. Os mesmos foram verificados e estudados quanto às possíveis formas de se criar e exibir páginas, além das configurações possíveis. ~\citetext{CAGIT}
...
Todo o conteúdo exibido ao se abrir a página do link criado para o pawtucket é descrito por arquivos feitos na linguagem PHP que ficam no diretório \textit{themes/mdjvi/views}. Nesta pasta foram configuradas todas as informações referentes ao museu e a exibição das mesmas para os usuários do site.
...
As figuras exibidas atrás desta pesquisa são de peças do acervo do Museu D. João VI. As imagens destas peças são exibidas em destaque para familiarizar o visitante do site com o conteúdo disponível para pesquisa e visita. Já para o logotipo do site foi usado o do próprio Museu, utilizado em sua página do Facebook.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item PESQUISA AVANÇADA - Exibe os campos que podem ser utilizados na pesquisa avançada. Também tem uma coluna com dicas de como realizar uma pesquisa;
	\end{itemize} 
	
	\item \textbf{ACERVO ARQUIVÍSTICO} - Direciona para a página contendo os itens importados do Acervo Arquivístico.
...
\begin{itemize}
	\item \textbf{Palavra-Chave} - A palavra ou texto inserido aqui será pesquisada em todo o acervo, similar à pesquisa múltipla ou pesquisa rápida da tela inicial. Pode ser inserido qualquer conteúdo do acervo independente de onde se encontre (título, localização, criador, etc.);
	\item \textbf{Título} - Limita a pesquisa do texto inserido a somente o título do objeto;
	...
	\item \textbf{Coleção} - Pesquisa objetos pertencentes à coleção especificada.
\end{itemize}
...
Já no final de todas as páginas foram acrescentados alguns links úteis: O site da Escola de Belas Artes da UFRJ, da Biblioteca de Obras Raras da EBA e o da Biblioteca Nacional, além do Facebook do Museu D. João VI e o email de contato.
...
Apesar de não ter sido o foco deste projeto, o conteúdo do Acervo Arquivístico foi importado para o programa. Como este material encontra-se digitalizado em um site externo, também seria interessante a criação de uma ligação entre a ficha do arquivo com a versão digitalizada do mesmo através de um \textit{link}.
